...
for subject in tqdm.tqdm(subjects):
    ...
    output_filename = f'./repertoires/all_repertoires_HC_completeinfo/healthy_repertoire_withcdrh3duplicates_{subject}.txt'
    subject_hp[['sequence_id', 'sequence_alignment_aa', 'label','cdr3_aa', 'v_identity','d_identity','j_identity','v_family', 'd_family','j_family','c_family']].to_csv(output_filename, index=False, header=None)
...
for subject in tqdm.tqdm(subjects):
    ...
    # Salva il file con il nome del soggetto
    output_filename = f'./repertoires/all_repertoires_HC_completeinfo/bnabs_repertoire_withcdrh3duplicates_{subject}.txt'
    subject_hp[['sequence_id', 'sequence_alignment_aa', 'label','cdr3_aa', 'v_identity','d_identity','j_identity','v_family', 'd_family','j_family','c_family']].to_csv(output_filename, index=False, header=None)
